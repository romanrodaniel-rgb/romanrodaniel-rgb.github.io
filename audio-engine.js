...
export class AudioEngine {
  ...
  async ensure() {
    ...
    if (this.ctx) {
      // Safari/iOS puede usar el estado no estándar `interrupted`, además de `suspended`.
      // Reanudamos siempre que el contexto no esté realmente activo.
      if (this.ctx.state !== 'running' && this.ctx.state !== 'closed') {
        try { await this.ctx.resume(); } catch (_) { /* Safari puede exigir otro toque del usuario. */ }
      }
      return this.ctx.state === 'running';
    }

    const AudioContextClass = window.AudioContext || window.webkitAudioContext;
    if (!AudioContextClass) throw new Error('Este navegador no ofrece Web Audio.');
    try {
    ...
    // La salida de grabación no existe en todas las versiones móviles de Safari.
    // La escucha debe seguir funcionando aunque esa API no esté disponible.
    ...
    if (this.ctx.state !== 'running' && this.ctx.state !== 'closed') {
      try { await this.ctx.resume(); } catch (_) { /* Se reintentará en el siguiente toque. */ }
    }
    return this.ctx.state === 'running';
  }
  ...
  primeMobileOutput() {
    ...
    // iOS necesita que una fuente se inicie dentro del gesto que habilita el audio.
    // El tono es prácticamente inaudible, pero abre la ruta de salida de Web Audio.
  ...
  async unlock() {
    ...
    await this.ensure();
    this.primeMobileOutput();
    if (this.ctx?.state !== 'running' && this.ctx?.state !== 'closed') {
      try { await this.ctx.resume(); } catch (_) { /* Safari reintentará con el siguiente toque. */ }
    }
    // No bloqueamos el gesto del usuario esperando la descarga del audio.
    // La precarga continúa en segundo plano y el primer nodo puede sonar en cuanto termine.
    const bell = this.bellAsset();
    if (bell) void this.getBuffer(bell.id, { priority: 'active' }).catch(() => null);
    return this.ctx?.state === 'running';
  }
  ...
  async playPlan(plan, generation) {
    ...
    // Safari puede volver a suspender/interrumpir el contexto durante la descarga o decodificación.
    await this.ensure();
